...
/**
 * This class encapsulates all functionality for draw
 ing the current view at the maze from a first person perspective.
...
public class FirstPersonDrawer extends DefaultViewer {
	 ...
	 */
	@Override
	public void redraw(int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang, float battery_level, int num_steps) {
...
//			return ;
	 ...
	 * @param nn
	 */
	protected void drawAllVisibleSectors(BSPNode nn) {
	 ...
	 * @param n
	 */
	protected void drawAllSegmentsOfASector(BSPLeaf n) {
	...
	private void drawSegment(Seg seg, int ox1, int y1, int ox2, int y2) {
		...
		int x1i = x1;
		int xd = x2-x1;
		mazepanel.setColor(seg.colorMe);
		boolean drawn = false;
