...
public class MazeBuilder implements Runnable {
	...
	public void run() {
		...
		try {
			...
			maxPercentDone = maze.getPercentdone();
			Thread.sleep(SLEEP_INTERVAL) ; // test if thread has been interrupted, i.e. notified to stop
	...
	public int getMaxPercentDone() {
		return maxPercentDone;
	}
	
	//call inside Playactivity
	public void callNewMaze() {
		maze.newMaze(root, cells, dists, startx, starty);
		Log.v("MazeBuilder", "Calling newMaze to build a new maze");
	}
