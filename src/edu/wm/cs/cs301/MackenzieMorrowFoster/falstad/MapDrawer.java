...
//		this.map_scale = map_scale ;
	...
	public void redraw(int state, int px, int py,
			int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang, float battery_level, int num_steps) {
		//dbg("redraw") ;
		if (state != Constants.STATE_PLAY)
			return ;
		if (maze.isInMapMode()) {
			draw_map(mazepanel, px, py, walk_step, view_dx, view_dy, maze.isInShowMazeMode(), maze.isInShowSolutionMode()) ;
	...
	public void draw_map(MazePanel mazepanel, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
		...
		for (int y = ymin; y <= ymax; y++)
			for (int x = xmin; x <= xmax; x++) {
				...
				mazepanel.setColor(seencells.hasWallOnTop(x, y) ? Color.WHITE : Color.GRAY);
				...
				mazepanel.setColor(seencells.hasWallOnLeft(x, y) ? Color.WHITE : Color.GRAY);
	...
	public void draw_solution(MazePanel mazepanel, int offx, int offy, int px, int py) {
		...
		while (d > 1) {
			...
			// do the graphics
			mazepanel.drawLine(nx1, ny1, nx1+ndx, ny1+ndy);
